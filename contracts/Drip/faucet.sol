...
contract Faucet is Ownable {
    ...
    function updateBeneficiary(address _beneficiary, uint256 _interval) public {
        ...
        require(
            (_interval >= 90 days && _interval <= 730 days),
    ...
    function updateHoldRequirements(uint256[] _newRefBalances)
        public
        onlyOwner
    {
    ...
    function deposit(address _upline, uint256 _amount) external {
        ...
        (uint256 realizedDeposit, uint256 taxAmount) = dripToken
    ...
    function checkForManager(address _addr) internal {
        ...
        require(
            custody[_addr].manager == msg.sender,
            "_addr is not set as manager"
        );
    }
    ...
    function isManager(address _addr, address _manager)
        public
        view
        returns (bool)
    {
    ...
    function isBeneficiary(address _addr, address _beneficiary)
        public
        view
        returns (bool)
    {
    ...
    function isManagementEligible(address _addr, address _manager)
        public
        view
        returns (bool)
    {
        return
            _manager != address(0) &&
            _addr != _manager &&
            users[_manager].deposits > 0 &&
            users[_manager].deposit_time > 0;
    }
    ...
    function airdrop(address _to, uint256 _amount) external {
        ...
        (uint256 _realizedAmount, uint256 taxAmount) = dripToken
            .calculateTransferTaxes(_addr, _amount);
